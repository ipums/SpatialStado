...
public class Optimizer {
            ...
            for (int i = 0; i < aQueryTree.getConditionList().size(); i++) {
                ...
                if (cond.isTwoRelationJoin()) {
                    ...
                    if (aRelationNode.getSysTable(database)
                    	...
                    	//sray
                    	if (cond.isSpatialTopologicalJoinPredicate()) {
                    		leftColumn = cond.getColumnList().get(0);
                    		rightColumn = cond.getColumnList().get(1);
                    	}
                    	else { 
    ...
    throws XDBServerException {
        ...
        try {
            ...
            for (int i = 0; i < joinList.size(); i++) {
                ...
                System.out.println("\n++++++++++++++++++++++++++++++++++++++++++++++++++\n In Optimizer The next tree :\n" + newQueryTree.toString()); //sray
                // add the tree to the next set.
                newTreeSet.addElement(newQueryTree.getRootNode().getEstCost(),
                        newQueryTree);
            }
    ...
    private void setConditionAtJoinNode(QueryTree targetTree,
        ...
        try {
            ...
            // sray: spatial join
            QueryNode leftChild = currentNode.getLeftNode();
            QueryNode rightChild = currentNode.getRightNode();
            
            if (leftChild.getNodeType() == QueryNode.RELATION 
            		&& (leftChild.getRelationNode().getSysTable(database).getPartitionScheme() == SysTable.PTYPE_SPATIAL)
            		) {
            	if (rightChild.getNodeType() == QueryNode.RELATION 
            			&& (rightChild.getRelationNode().getSysTable(database).getPartitionScheme() == SysTable.PTYPE_SPATIAL)
            			) {
            		String leftPartitionSchemaId = leftChild.getRelationNode().getSysTable(database).getSpatialPartId();
            		
            		String rightPartitionSchemaId = rightChild.getRelationNode().getSysTable(database).getSpatialPartId();
            	
            		if (leftPartitionSchemaId.equals(rightPartitionSchemaId))
            			currentNode.setSpatialJoin(true);
            	}
            }

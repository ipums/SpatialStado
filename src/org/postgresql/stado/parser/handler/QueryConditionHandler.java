    ...
    public Object visit(SQLRelationalExpression n, Object argu) {
        ...
        case 1:
            ...
            if (sqlh.aroot.getExprType() == SqlExpression.SQLEX_CONDITION) {
                ...
                //sray
                if (  sqlh.aroot.getFunctionId()== IFunctionID.ST_INTERSECTS_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_OVERLAPS_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_TOUCHES_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_CROSSES_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_CONTAINS_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_CONTAINSPROPERLY_ID // stay:TODO remove?
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_COVEREDBY_ID 
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_WITHIN_ID
                	|| sqlh.aroot.getFunctionId()== IFunctionID.ST_DISJOINT_ID
                		)
                	((QueryCondition) sqlhelper.getArgument()).setSpatialTopologicalJoinPredicate(true);

...
public class NodeProducerThread implements Runnable {
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    //sray
                    case  NodeMessage.MSG_EXECUTE_STEP_SPATIAL_JOIN:
                    	checkCurrentState(new int[] { STATEWAIT }, STATESTEP);
                        try {
                        	 if (aNodeMessage.getSpatialPartitionId() == 179 || aNodeMessage.getSpatialPartitionId() == 180 || aNodeMessage.getSpatialPartitionId() == 1023 || aNodeMessage.getSpatialPartitionId() == 1024)
                             	System.out.println("wait here.."); 
                        	 
                        	long start= System.currentTimeMillis();
                            //ResultSet aResultSet = processStepsAtNodesThenSendtoCoord(aNodeMessage);
                        	ResultSet aResultSet = processStepsAtNodesThenSendtoCoordSQL(aNodeMessage);
                        	if (aResultSet != null)
                        		aResultSet.close();
                        	QueryResult aQueryResult = null;
                        	
                            long end= System.currentTimeMillis();
                            if ( ((end-start)/(1000*60))  >  1) // more than 2 minutes
                            	System.out.println("Took more than 1 minutes " + aNodeMessage.getStepDetail().queryString);
                           ...
                           // if (aNodeMessage.getSpatialPartitionId() == 179 || aNodeMessage.getSpatialPartitionId() == 180 || aNodeMessage.getSpatialPartitionId() == 1023 || aNodeMessage.getSpatialPartitionId() == 1024)
                            // 	System.out.println("Step is done, ResulSet: " + aResultSet);
        ...
        try {
            ...
            try {
            	...
            	if (Props.XDB_USE_COPY_OUT_FOR_STEP && aStepDetail.getDestType() != StepDetail.DEST_TYPE_COORD_FINAL) {
            		...
            		CopyOut copyOut = copyManager.copyOut("copy (" + aStepDetail.queryString + ") to stdout"); //sray: here copy (SELECT query ) to stdout gets executed
    ...
    private ResultSet processStepsAtNodesThenSendtoCoord(NodeMessage aNodeMessage)
    	    throws XDBBaseException {
    	        final String method = "processStepsAtNodesThenSendtoCoord";
    	        logger.entering(method);
    	        try {
    	            ...
    	            try {
    	            	...
    	            	long start= System.currentTimeMillis(); //sray:TODO
    	            		
    	            		CopyManager copyManager = CopyManager.getCopyManager(oConn);
    	            		CopyOut copyOut = copyManager.copyOut("copy (" + aStepDetail.queryString + ") to stdout");
    	            		
    	            	long end= System.currentTimeMillis(); //sray:TODO
                        if ( ((end-start)/(1000*60))  >  1) // more than 2 minutes
                           	System.out.println("Took more than 1 minutes @ processStepsAtNodesThenSendtoCoord:  " + aNodeMessage.getStepDetail().queryString);
                
    	            		
    		                aProducerSender = new ProducerSender(sendHelper, producerQueue);
    		                aProducerSender.execAtNodesThenSendtoCoord(copyOut, aStepDetail, oConn,
    		                        nodeId, aNodeMessage.getRequestId());
    	            	}
    	            	else {
    	            		System.err.println("ERROR: IN processStepsAtNodesThenSendtoCoord - COPY not supported ");
    	            	}
    ...
    //sray
    private ResultSet processStepsAtNodesThenSendtoCoordSQL(NodeMessage aNodeMessage)
    	    throws XDBBaseException {
    	        final String method = "processStepsAtNodesThenSendtoCoord";
    	        logger.entering(method);
    	        try {
    	            ...
    	            try {
    	            	...
    	            	long start= System.currentTimeMillis(); //sray:TODO
	    	                ...
	    	                synchronized (oConn) {
	    	                    ...
	    	                    // Fix for bug: org.postgresql.stado.exception.XDBDataReaderException: Cancelled by user
	    	                    aStatement = oConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);   
	    	                ...
	    	                synchronized (oConn) {
	    	                    ...
	    	                    //}
	    	                    aResultSet = aStatement.executeQuery(aStepDetail.queryString);
	    	                    
	    	                    //sray: toRemove
	    	                    
	    	                    aResultSet.last();
	    	                    
	    	                    int numResults = aResultSet.getRow();
	    	                    if (numResults>0 && Props.XDB_TO_DEBUG)
	    	                    	System.out.println("NodeProducerThread.processStepsAtNodesThenSendtoCoordSQL: numResultRows: " + numResults + "  query:" + aStepDetail.queryString);
	                      ...
	                      if ( ((end-start)/(1000*60))  >  1) // more than 2 minutes
	    		                ...
	    		                aProducerSender = new ProducerSender(sendHelper, producerQueue);
	    		                aProducerSender.execAtNodesThenSendtoCoordSQL(aResultSet, aStepDetail, oConn,
	    		                        nodeId, aNodeMessage.getRequestId());
    ...
    private void dropTempTables(Collection dropList, boolean autocommit) {
        ...
        try {
            ...
            for (Iterator it = dropList.iterator(); it.hasNext();) {
                ...
                try {
                	System.out.println("NodeProducerThread.dropTempTables: DROP TABLE "+dropName); //sray
    ...
    throws XDBBaseException {
        ...
        try {
            ...
            synchronized (oConn) {
                ...
                aStatement = oConn.createStatement();
                try {
                	if (Props.XDB_TO_DEBUG) System.out.println("NodeProducerThread.processSqlCommand: "+sqlCommandStr); //sray

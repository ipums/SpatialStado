...
//sray: toRemove
if (Props.XDB_TO_DEBUG) System.err.println("Buffer capacity " + capacity);
	...
	*/
    //sray:  toRemove
    public synchronized void markFinished(int callingThreadId) {
    	if (Props.XDB_TO_DEBUG) System.out.println("####DataReaderAndProcessorBuffer in markFinished: threadId="+ callingThreadId + " marking buffer finished\n");
    ...
    */
     ...
     */
    //sray: toRemove
    public synchronized void putRowValue(T value, String hashString, int callingThreadId)
    throws XDBDataReaderException {
        try {
            while ( !hasFinished && currentSize == bufferLimit ) {
                try {
            ...
            if (hasFinished) {
            	System.err.println("####DataReaderAndProcessorBuffer in putRowValue: threadId="+ callingThreadId + " throwing exception\n");
                throw new XDBDataReaderException("Cancelled by user");
            }
            if ( in==bufferLimit ) { in = 0 ; }
            currentSize++ ;
    ...
    public synchronized void putRowValue(T value) throws XDBDataReaderException {
        try {
        	...
        	if (Props.XDB_TO_DEBUG) System.out.println("DataReaderAndProcessorBuffer in putRowValue: threadId="+ threadId +"\n");
        	putRowValue(value, null, threadId); //sray: toRemove
        }
        finally {
            notify() ;
        }

...
public class Loader implements ILoaderConfigInformation {
    ...
    static {
        //executorService = Executors.newCachedThreadPool(); //sray:TODO determine the best thread pool for loader
    ...
    public void runWriters() throws Exception {
        ...
        if (dataSource == DATA_SOURCE.RESULT_SET) {
            ...
            for (int i = 0; i < dpThreads.length; i++) {
                ...
                dpThreads[i].initializeDataWriters();
                dpHandles[i] = executorService.submit(dpThreads[i]); //sray: here the COPY "TMP" from STDIN WITH .. gets executed
            }
        } else {

...
public abstract class DataProcessorThread<T> implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        try {
        	...
        	if (Props.XDB_TO_DEBUG) System.out.println("$$$$DataProcessorThread in call: threadId="+ this.threadId + " marking buffer finished\n");
            loadBuffer.markFinished(this.threadId); //sray: toRemove
            throw ex;
        }
    }
    ...
    public void cancel() {
    	//loadBuffer.markFinished(); //sray: toRemove
    	
    	if (Props.XDB_TO_DEBUG) System.out.println("$$$$DataProcessorThread in cancel: threadId="+ this.threadId + " marking buffer finished\n");
        loadBuffer.markFinished(this.threadId);
    }
}

...
public class MessageMonitor {
    ...
    //sray: keep track of messages by spatial partition id, rather than the node id
    ...
    public synchronized void setMonitor(int requestId, Collection sources)
        ...
        try {
            ...
            if (sources != null) {
                ...
                messages = new ArrayList<NodeMessage>(sourceList.size()); //sray: change the size later if a spatial join
            } else {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------------------------
    //sray: by spatial partition id - next 3 methods
    
    
    public synchronized void registerSpatialPartionId(int partitionId)
            throws XDBServerException {
        final String method = "registerSpatialPartionId";
        logger.entering(method,
                new Object[] { new Integer(partitionId) });
        try {
            ...
            if (partitionList == null) {
                ...
                messages = new ArrayList<NodeMessage>(); // change the  size
            }
     ...
     *            received message
     */
    public synchronized void registerMessagesBySpatialPartitionId(NodeMessage msg) {
        final String method = "registerMessagesBySpatialPartitionId";
        logger.entering(method, new Object[] { msg });
        try {

            if (sourceList != null  && partitionList!= null && msg.getRequestId() == currentRequestId) {

                // Check to see if it is an informational message about sending
                // rows
                if (partitionList.remove(new Integer(msg.getSpatialPartitionId()))) {
                    logger.debug("Getting closer: " + partitionList);
                   // System.out.println("Received msg reply for partition "+ msg.getSpatialPartitionId());
                    messages.add(msg);
                    notify();
                }
            }
    ...
    public synchronized NodeMessage[] checkMessagesBySpatialPartitionId() throws XDBServerException {
        // final String method = "checkMessages";
        // logger.entering(method);
        try {
            ...
            try {
                ...
                // condition to return the messages

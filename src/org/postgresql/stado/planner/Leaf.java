...
public class Leaf {
    ...
    //sray
    public LinkedHashMap<String,Projection> selectColumnMapOrig;

    /** whether or not DISTINCT should be applied here. */
    private boolean distinct = false;

    //sray
    private boolean isSpatialJoin = false;
    private boolean isSpatialJoinDuplicateEliminationStep = false;
    /**
    ...
    public Leaf() {
        ...
        //sray
        selectColumnMapOrig = new LinkedHashMap<String,Projection>();
    ...
    //sray
    public void appendProjectionOrig(
            final String exprStringOrig) {

       
        // the create column name
        Projection aColumn = this.new Projection(exprStringOrig);
        selectColumnMapOrig.put(aColumn.projectString, aColumn);            
    ...
    //sray
    public String appendProjectionFromExprOrig(final SqlExpression aSqlExpression,
            final boolean isFinalStep) {
    	String columnName;
    ...
    //sray
    public boolean isProjectionOrig(final String projectionStringOrig) {
        // See if it already is in Leaf.
        if (projectionStringOrig == null) {
            return false;
        }        
        return selectColumnMapOrig.get(projectionStringOrig) != null;
    }
    ...
    public void determineSelectStatement(QueryPlan aQueryPlan) {
        ...
        if (sbFromOn.indexOf(" ON ") < 0) {
        	...
        	if (Props.XDB_SQL_USECROSSJOIN && !isSpatialJoin) { //sray
        ...
        //sray: For iterative execution
        if (Props.XDB_SPATIAL_JOIN_ITERATIVE_EXEC && isSpatialJoin) {
        	//isSpatialJoin
        	for (int i = 0; i < extendedAliasList.size(); i++) {
        		String sAlias = extendedAliasList.get(i);
        		whereClause += " AND " + "\"" + sAlias + "\"" + "."  + "\"" + Props.XDB_SPATIAL_JOIN_SHARDID_COLUMN_NAME  + "\" = " + Props.XDB_SPATIAL_JOIN_SHARDID_REPL_SUBSTR; 
        	}
        }
...
//        selectColumnMap.put(aColumn.projectString, aColumn);
    ...
    public void setSelectStatement(String selectStatement) {
        ...
        //sray
        System.out.println("\n ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n leaf.setSelectStatement:" + selectStatement);
    }
    ...
    public boolean isSpatialJoin() {
    	return isSpatialJoin;
    }
    public void setSpatialJoin(boolean val) {
    	isSpatialJoin = val;
    }
    public boolean isSpatialJoinDuplicateEliminationStep() {
    	return isSpatialJoinDuplicateEliminationStep;
    }
    public void setSpatialJoinDuplicateEliminationStep(boolean val) {
    	isSpatialJoinDuplicateEliminationStep = val;
    }

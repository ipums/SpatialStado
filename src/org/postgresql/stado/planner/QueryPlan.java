...
import org.postgresql.stado.optimizer.FunctionAnalysis;
...
public class QueryPlan
{
     ...
     *
     * sray: extended to handle spatial join 
     * 
    ...
    private void createPlanStep(QueryTree aQueryTree, QueryNode aQueryNode)
        ...
        try {
            ...
            //sray: If parent node contains spatial join condition then 
            // don't create a step.
            if (aQueryNode.getParent() != null && aQueryNode.getNodeType() == QueryNode.RELATION
                    && aQueryNode.getParent().isSpatialJoin()) {
            ...
            if (aQueryNode.getNodeType() == QueryNode.RELATION
                ...
                if (!aQueryNode.isTreeDownJoin()) {
                    ...
                    //sray: include left table name for spatial join
                    if (aQueryNode.isSpatialJoin()) {
                    	aLeaf.setSpatialJoin(true);
                    	QueryNode leftNode = aQueryNode.getLeftNode();
                    	aLeaf.extendedOuterLevel.add(Integer.valueOf(
                    			leftNode.getOuterLevel()));

                    	if (leftNode.isSubtreeOuter()) {
                    		aLeaf.extendedOuter.add(Boolean.TRUE);
                    	} else {
                    		aLeaf.extendedOuter.add(Boolean.FALSE);
                    	}
                          
	                    aLeaf.extendedTableList.add(leftNode.getRelationNode().getTableName());
	                    if (leftNode.getRelationNode().getAlias() != null
	                            && leftNode.getRelationNode().getAlias().length() > 0) {
	                        aLeaf.extendedAliasList.add(leftNode.getRelationNode().getAlias());
	                    } else {
	                        aLeaf.extendedAliasList.add(leftNode.getRelationNode().getTableName());
	                    }
	                    aLeaf.addUsedTable(leftNode.getRelationNode().getTableName());
                    }
                }
            ...
            // sray: spatial join duplicate elimination: add a TMP table select distinct step
            if (aQueryNode.isSpatialJoin()
            		&& !aQueryTree.isContainsAggregates()) { //does it work for all cases?
            	//aLeaf.setCombineOnMain(true);
           	
            	// Create a new Step
                Leaf tsdLeaf = getStepToProcess (aQueryNode, true);
                tsdLeaf.setSpatialJoinDuplicateEliminationStep(true);
                createPlanStepDoProjections(aQueryTree, aQueryNode, tsdLeaf);
                
                //tsdLeaf.setCombinerStep(true);
                tsdLeaf.setTableName(aLeaf.getTargetTableName());
            	tsdLeaf.setTargetTableName(generateTempTableName());
            	//tsdLeaf.setTargetTableName(aLeaf.getTargetTableName());
            	
                tsdLeaf.extendedOuterLevel.add(0);
                tsdLeaf.extendedOuter.add(Boolean.FALSE);
            	tsdLeaf.extendedTableList.add(tsdLeaf.getTableName());
            	tsdLeaf.extendedAliasList.add(tsdLeaf.getTableName());       
            	tsdLeaf.addUsedTable(tsdLeaf.getTableName());
            	//tsdLeaf.setCombineOnMain(true);
            	//tsdLeaf.setExtraStep(true);
            	
            	// specify what temp table to join with for next step
            
            	lastJoinTableName = tsdLeaf.getTargetTableName();
            	updateQueryNodeTempTableName(aQueryNode, tsdLeaf.getTargetTableName());
        ...
        try {
            ...
            if (aQueryNode.getParent() == null
                    && (!aQueryTree.isContainsAggregates()
                            || aQueryTree.isPartitionedGroupBy() || aQueryTree.usesSingleDBNode(database))
                    && !aQueryTree.isCorrelatedSubtree()
                    && !aLeaf.isSpatialJoinDuplicateEliminationStep())  { //sray 
                ...
                for (SqlExpression aSqlExpression : aQueryNode.getProjectionList()) {
                    ...
                    //sray
                    if (aSqlExpression.getExprStringOrigTableAndCol() != null) { 
                    	if (!aLeaf.isProjectionOrig(aSqlExpression.getExprStringOrigTableAndCol())
                                || aSqlExpression.containsAggregates()
                                || aSqlExpression.isTempExpr()) {
                    		if (aLeaf.isProjection(aSqlExpression.getExprString())) {
                    			aSqlExpression.rebuildExpressionWithColumnAlias();
                    			aLeaf.appendProjectionFromExprOrig(aSqlExpression, false);
                    		}
                    		else 
                    			aLeaf.appendProjectionFromExpr(aSqlExpression, false);
                            aLeaf.appendProjectionOrig(aSqlExpression.getExprStringOrigTableAndCol()); //sray
                    	}
                    }
                    else 
                    	if (!aLeaf.isProjection(aSqlExpression.getExprString())
    ...
    private void updateFetchProjections(QueryTree aQueryTree, Leaf aLeaf) {
        final String method = "updateFetchProjections";
        logger.entering(method);

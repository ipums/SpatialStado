...
public class StepDetail implements Serializable {
    ...
    //sray
    public String queryStringOrig = null; //sray:TODO see if to remove
    ...
    //sray
    public boolean isSpatialJoin = false;
    public boolean isSpatialJoinCoord = false;
    public boolean isSpatialJoinDuplicateEliminationStep  = false;
    ...
    public StepDetail copy() {
        ...
        newStepDetail.queryStringOrig = this.queryStringOrig;//sray:TODO remove?
    ...
    public void convertLeafToStep(Leaf aLeaf, Leaf nextLeaf, Leaf nextNextLeaf,
        ...
        try {
            ...
            if (nextLeaf == null) {
            	...
            	//sray
            	if (aLeaf.isSpatialJoin()) {
            		
            		this.isConsumer = false;
            	}
            	else if (aLeaf.isSpatialJoinDuplicateEliminationStep()) {
            		
            		this.isConsumer = true;
            	}
            	else if (!(nextLeaf.isCombineOnMain() || aLeaf.isCombinerStep() || aLeaf.isExtraStep())) {
                    this.isConsumer = true;
                } else {
                    this.isConsumer = false;
            ...
            //sray
        	if (aLeaf.isSpatialJoin()) {
        		this.isSpatialJoin=true;
        		this.isProducer = true; 
        		//this.combineOnCoordFirst = true;
        		//this.destNodeList = aLeaf.queryNodeList;
        	}
        	else if (aLeaf.isSpatialJoinDuplicateEliminationStep()) {
        		this.isSpatialJoinDuplicateEliminationStep = true;
        		this.isProducer = false; 
        	}
        	else if (!(aLeaf.isCombineOnMain() || aLeaf.isCombinerStep() || aLeaf.isExtraStep())) {
                this.isProducer = true;
            }
            else {
                this.isProducer = false;
        	...
        	//sray
        	this.srcTable = aLeaf.getTableName(); // needed for duplicate elimination
    ...
    public void convertLeafToStepCoord(Leaf aLeaf, Leaf nextLeaf,
        ...
        try {
            ...
            //sray
        	if (aLeaf.isSpatialJoin()) {
        		this.isProducer = false; //was true, set false
        	}
        	else if (aLeaf.isSpatialJoinDuplicateEliminationStep()) {
        		if (nextLeaf == null) {
        			this.isProducer = true;
        			...
        			this.isProducer = false; //was false, set true
        	}
        	else if (aLeaf.isCombineOnMain() || aLeaf.isCombinerStep()
                    || aLeaf.isExtraStep()) {
                this.isProducer = true;
            ...
            // update StepDetail's producer info appropriately.
            if (this.isProducer) {
            	//sray: select result from coord after duplicate elimination 
            	if (nextLeaf == null && aLeaf.isSpatialJoinDuplicateEliminationStep()) {
            		this.targetSchema = "";
            		updateProducerStepInfoForSpatialJoinDuplicateEliminationCoord(aLeaf, nextLeaf, nextNextLeaf, this);
            	}
            	else 
            		updateProducerStepInfo(aLeaf, nextLeaf, nextNextLeaf, this);
            }
    ...
    //sray
    private void updateProducerStepInfoForSpatialJoinDuplicateEliminationCoord(Leaf aLeaf, Leaf nextLeaf,
            Leaf nextNextLeaf, StepDetail aStepDetail) {
        final String method = "updateProducerStepInfoForSpatialJoinDuplicateEliminationCoord";
        logger.entering(method);

        try {
            SysTable sysTab = null;

            aStepDetail.queryString = "select * from " + "\""+ aStepDetail.targetTable + "\"";
    ...
    // sray
    public int getCoordNodeId() {
    	return client.getCoordNodeId();
    }
}

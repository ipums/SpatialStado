...
public class ExecutionStep {
    ...
    //sray
    public boolean isFinalStepSpatialJoinDuplicateElimination = false;
    ...
    protected void convertFromLeaf(Leaf currentLeaf, Leaf nextLeaf,
        ...
        try {
            ...
            if (aStepDetail.isSpatialJoinDuplicateEliminationStep) {
            	consumerCount = previousLeaf.queryNodeList.size();
            	aStepDetail.isConsumer = true;
            	aStepDetail.consumerNodeList = new ArrayList<Integer>();

                for (DBNode dbNode : previousLeaf.queryNodeList) {
            ...
            else if (aStepDetail.isConsumer) {
    ...
    protected void correctDestinations(ExecutionStep nextStep) {
        ...
        if (this.aStepDetail.isProducer) {
            ...
            if (coordStepDetail!=null && !nextStep.origLeaf.extendedTableList.contains(coordStepDetail.targetTable)) { //sray coordStepDetail!=null &&
                return;
            }
        }
            	...
            	*/
            	//else {
	                ...
	                if (aStepDetail.combineOnCoordFirst) {
	                	...
	                	Collection<DBNode> dbNodeList = database.getDBNodeList();
	                    for (DBNode aDBNode : dbNodeList) {
	                    	for (int nodeId : aStepDetail.consumerNodeList) {
	                    		if (nodeId == aDBNode.getNodeId())
	                    		destNodeList.add(aDBNode);
	                    	}
    ...
    //sray
    public void setDestNodeListForSpatialJoinDuplicateElimination() {

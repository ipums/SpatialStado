...
public class SyncCreateTable implements IMetaDataUpdate {
    ...
    private String spatialPartitionIdentifier;
    private Integer spatialPartitionCount;
    private Integer spatialPlacement;
    private Integer spatialLevel;
    ...
    public SyncCreateTable(SqlCreateTable aSqlCreateTable) {
        ...
        spatialPartitionIdentifier = aSqlCreateTable.getSpatialPartitionIdentifier();
        //if (spatialPartitionIdentifier==null) spatialPartitionIdentifier="";
        spatialPlacement = aSqlCreateTable.getSpatialPlacement();
        if (spatialPlacement==null) spatialPlacement=NONE;
        spatialPartitionCount = aSqlCreateTable.getSpatialPartitionCount();
        if (spatialPartitionCount==null) spatialPartitionCount=NONE;
        spatialLevel = aSqlCreateTable.getSpatialLevel(); 
        if (spatialLevel==null) spatialLevel=NONE;
    }
    ...
    public void execute(XDBSessionContext client) throws Exception {
        ...
        sqlStatement = "insert into xsystables"
                ...
                + ","
                + (spatialPlacement == NONE ? "null" : spatialPlacement.toString())
                + ","
                + (spatialPartitionIdentifier == null ? "null" : "'" + spatialPartitionIdentifier + "'") 
                + ","
                + (spatialPartitionCount == NONE ? "null" : spatialPartitionCount.toString())
                + ","
                + (spatialLevel == null ? NONE : spatialLevel.toString())
                + ") RETURNING tableid";
        ...
        if (partScheme == SysTable.PTYPE_SPATIAL) {
        	// sray:TODO temporarily store in metadata table xsystabparthash. Later on, use a new table xsystabpartspatial
        	partMap.storeMapToMetadataDBSpatial(MetaData.getMetaData(), database, tableid);
        }
        else {
	        if (parentTable == null) {
	            // ----------------------------------
    ...
    //sray
    public void executeForInherited(XDBSessionContext client, String InhTableName) throws Exception {
        this.client = client;
        database = MetaData.getMetaData().getSysDatabase(client.getDBName());
    ...
    public void refresh() throws Exception {
        SysTable aSysTable = new SysTable(database, tableid, tableName, 0, 
                ...
                spatialPlacement,spatialPartitionIdentifier,spatialPartitionCount,spatialLevel); //sray

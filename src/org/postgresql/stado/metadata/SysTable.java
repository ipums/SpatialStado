...
public class SysTable {
    ...
    public static final short PTYPE_SPATIAL = 7; //sray: Spatial partition
    ...
    /**
     *sray: for spatial join  
	 spatialplacement | smallint              | 
 	 spatialpartid    | character varying(80) | 
     spatialpartcount | integer               | 
     spatiallevel     | integer       
     */
    private int spatialPlacement;
	private String spatialPartId;
    private int spatialPartCount;
    private int spatialLevel;
    ...
    public SysTable(SysDatabase database, int tableID, String tableName,
            ...
            int splplacement, String splpartid, int splpartcount, int spllevel ) { 
        ...
        if (partitionScheme == SysTable.PTYPE_LOOKUP) {
            ...
            partitionMap = new ReplicatedPartitionMap();
        } else if (partitionScheme == SysTable.PTYPE_SPATIAL) {
            partitionMap = new HashPartitionMap(); //sray:TODO : for now use the HashPartitionMap.  Later on use SpatialPartitionMap
        }
        ...
        //sray: spatial partition related
        spatialPlacement = splplacement;
        spatialPartId = splpartid;
        spatialPartCount = splpartcount;
        spatialLevel = spllevel;
    }
    ...
    public SysColumn getPartitionedColumn() {
        if (parentTable == null) {
            if (partitionScheme == PTYPE_HASH || partitionScheme == PTYPE_RANGE 
            		|| partitionScheme == PTYPE_SPATIAL) { //sray
    ...
    public Collection<DBNode> getNode(String sValue) {
    	if (partitionScheme == SysTable.PTYPE_SPATIAL) { 
    		//sray:  ALL: all nodes
    		
    		Collection<Integer> allPartitions = partitionMap.allPartitions();
    		if (allPartitions.size()==0)
    		try {			
    				partitionMap.readMapFromMetadataDB(MetaData.getMetaData(), this);
    				allPartitions = partitionMap.allPartitions();
    		} catch (SQLException e) {
    	            throw new XDBServerException(
    	                    ErrorMessageRepository.METADATA_DB_INFO_READ_ERROR, e,
    	                    ErrorMessageRepository.METADATA_DB_INFO_READ_ERROR_CODE);
    		} finally {
    	        	logger.exiting("getNode");
    		}
    		return nodeIDs2DBNodes(allPartitions);
    		
    		//ROUND ROBIN: sray:TODO later
    		
    		
    		//Exotic: sray:TODO later
    ...
    // sray
    public int getSpatialPlacement() {
		return spatialPlacement;
	}

	public void setSpatialPlacement(int spatialPlacement) {
		this.spatialPlacement = spatialPlacement;
	}

	public String getSpatialPartId() {
		return spatialPartId;
	}

	public void setSpatialPartId(String spatialPartId) {
		this.spatialPartId = spatialPartId;
	}

	public int getSpatialPartCount() {
		return spatialPartCount;
	}

	public void setSpatialPartCount(int spatialPartCount) {
		this.spatialPartCount = spatialPartCount;
	}

	public int getSpatialLevel() {
		return spatialLevel;
	}

	public void setSpatialLevel(int spatialLevel) {
